...
testthat::test_that("mssql_server - upload method", {
  ...
  upload_outcome <- mssql_serv$upload(
    data = test_data,
  ...
  # n rows
  table_rows <- mssql_serv$get(glue::glue("SELECT count(*) as n
  ...
  # field names
  table_fields <- mssql_serv$get(glue::glue("SELECT TOP 0 *
                                    from {test_table_name}"),
    close_conn = FALSE
  )
  ...
  batch_upload_outcome <- mssql_serv$upload(
    data = test_data,
    table_name = test_table_name,
    batch_upload = 50,
    append_data = TRUE,
...
testthat::test_that("mssql_server - get data", {
  ...
  db_data <- mssql_serv$get(glue::glue("SELECT * FROM {test_table_name}"),
    close_conn = FALSE
  ) %>%
    dplyr::mutate(date_field = as.Date(date_field))
...
testthat::test_that("mssql_server - run & order by fields", {
  ...
  mssql_serv$run(glue::glue("ALTER TABLE {test_table_name}
  ...
  testthat::expect_error(
    suppressWarnings(mssql_serv$get(glue::glue("SELECT * FROM {test_table_name}"),
      close_conn = FALSE
    ))
  )
  ...
  table_fields <- mssql_serv$order_object_fields(
    object = test_table_name,
    close_conn = FALSE
  )

  # now extract all the data
  extract_ordered_feilds <- mssql_serv$get(glue::glue("SELECT {table_fields}
                                                   FROM {test_table_name}"),
    close_conn = FALSE
  ) %>%
    dplyr::mutate(date_field = as.Date(date_field))
  ...
  testthat::expect_equal(
    colnames(extract_ordered_feilds),
    c(
      "Int_field", "char_field_2", "date_field",
...
testthat::test_that("mssql_server - meta data", {
  ...
  table_fields <- mssql_serv$object_fields(
    objects = test_table_name,
    close_conn = FALSE
  )
  ...
  # get meta data
  meta_data <- mssql_serv$meta_data(
    objects = test_table_name,
    detail = TRUE,
    close_conn = FALSE
  )[[1]]
  ...
  testthat::expect_equal(
    c("date", "datetime", "int", "varchar", "varchar"),
  ...
  testthat::expect_equal(
    colnames(meta_data),
    c(
      "col_name", "data_type", "col_type", "nullable",
  ...
  meta_data2 <- mssql_serv$meta_data(
    objects = test_table_name,
    detail = TRUE,
    row_limit = 1000,
  ...
  testthat::expect_equal(meta_data2$n_rows[1], nrow(stats::filter(test_data, date_field >= date_to_filter)) * 2)
  ...
  # get meta data
  meta_data3 <- mssql_serv$meta_data(
    objects = test_table_name,
    detail = TRUE,
    close_conn = FALSE
  )[[1]]
...
testthat::test_that("mssql_server - close connection", {
  ...
  testthat::expect_error(
    mssql_serv$get(glue::glue("SELECT * FROM {test_table_name}"))
  )

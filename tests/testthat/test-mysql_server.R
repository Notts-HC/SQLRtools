...
testthat::test_that("mysql_server - upload method", {
  ...
  upload_outcome <- mysql_serv$upload(
    data = test_data,
  ...
  batch_upload_outcome <- mysql_serv$upload(
    data = test_data,
    table_name = test_table_name,
    schema_name = get_env_var("MYSQL_DB"),
    batch_upload = 10,
...
testthat::test_that("mysql_server - meta data", {
  ...
  # get meta data
  meta_data2 <- mysql_serv$meta_data(
    objects = test_table_name,
    detail = TRUE,
    close_conn = FALSE
  )[[1]]
